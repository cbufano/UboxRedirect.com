...
> **Atualização 30/07/2026 — Fase 7 / Onda 7.2 entregue (pendências & dinheiro):** o `/admin` virou o **Painel de Pendências** ("o que precisa de ação hoje": a conferir, pagas sem envio, LGPD, armazenagem vencida, conciliação — com estado "tudo em dia"); tela de **Pagamentos** (histórico com filtros, **pagamento manual** PIX/transferência com trava de RLS que impede forjar pagamento Stripe, registro de **estornos** com teto e anti-duplicidade); tela de **Tarifas & taxas** (CRUD de `rate_tables` + as 5 taxas de serviço migradas do texto fixo do site + **simulador** usando o cálculo real); tela de **Settings** (dias de armazenagem grátis, prazo de alerta, status das cotações); **multi-moeda de exibição** (USD é a única moeda real; "≈ R$" nos envios do cliente via cotação diária da Edge Function `refresh-exchange-rates` — deploy/agendamento no checklist manual). Revisão de segurança financeira: **aprovada**, 0 Críticos; achado Importante (teto/duplicidade de estorno) corrigido na hora; policy de `rate_tables` corrigida para incluir super_admin (aplicada em produção). Dívida registrada para a 7.3: proteção de colunas em `refunds` junto com o audit_log.

> **Atualização 30/07/2026 — Fase 7 / Onda 7.3 entregue (pessoas):** tela de **Clientes** (`/admin/customers` — busca por nome/e-mail/suite, perfil completo com pacotes/envios/pagamentos/LGPD, **notas internas** invisíveis ao cliente, **suspensão de conta** com bloqueio no painel do site, e os controles de KYC/OFAC movidos do recebimento para cá); tela de **Staff** (`/admin/staff` — promover/rebaixar papéis pela tela via RPC com regras de segurança: ninguém muda o próprio papel, mexer em admin exige super_admin; visualização do **log de auditoria**); **audit log imutável gravado por triggers do banco** (pagamentos, estornos, status, compliance, suspensão, papéis — impossível contornar via API; nem admin edita/apaga). Dívida da 7.2 paga: colunas de `refunds` congeladas após criação. Revisão de segurança: **aprovada**, 0 Críticos, nenhum caminho de escalada de privilégio; os 3 endurecimentos recomendados (revoke do write_audit, congelar refunds.id, lock anti-corrida + papel anterior no set_user_role) aplicados em produção na hora (migration `20260731000006`).

> **Atualização 30/07/2026 — Fase 7 / Onda 7.4 entregue (automação) — FASE 7 COMPLETA:** o "funcionário invisível": **outbox de e-mails gravado por triggers do banco** (pacote recebido, pronto para envio, pagamento confirmado, envio despachado — o código do site não consegue esquecer de notificar) + aviso de **armazenagem vencendo** gerado pela função de envio (1×/pacote a cada 7 dias); Edge Function `send-emails` (cron; com `RESEND_API_KEY` envia via Resend, sem ela marca "skipped" visível no admin; **claim atômica antes do envio** — execuções concorrentes nunca mandam o mesmo e-mail 2×; exige a service_role key no header, a anon key pública não dispara envios); **webhook genérico de rastreio** (`tracking-webhook`, secret próprio timing-safe, fail-closed) que grava `tracking_events` e **marca entrega sozinho** (delivered → consolidação `shipped`→`delivered` via trigger); exceções de rastreio e e-mails falhados viraram cartões no Painel de Pendências; painel do outbox em `/admin/settings`; **linha do tempo de rastreio** na tela de envios do cliente. Revisão final: aprovada com correções — 2 Críticos (envio duplicado sob concorrência; função invocável com a anon key) e 2 Importantes (reenvio de "package_ready" ao cancelar consolidação; `tracking_code` sem unicidade) corrigidos na hora, migration `20260731000008` aplicada em produção. **Com isso a Fase 7 (back-office) está completa: uma pessoa opera o negócio inteiro pelo `/admin`.**
...
- [ ] Agendar `send-emails` no Dashboard do Supabase (Integrations → Cron, a cada 15 min) **com o header `Authorization: Bearer <service_role_key>`** — a função rejeita qualquer outra credencial (a anon key pública não serve, de propósito).
